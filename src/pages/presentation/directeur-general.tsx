...
// src/pages/presentation/directeur-general.tsx (VERSION CORRIGÉE)
...
// --- Composant pour formater le texte avec une structure améliorée ---
const StructuredBio = ({ content }: { content: string }) => {
  if (!content) return null;

  // Fonction pour formater le texte avec une structure améliorée
  const formatContent = (text: string) => {
    ...
    return paragraphs.map((paragraph, index) => {
      ...
      if (trimmed.startsWith('-') || trimmed.startsWith('•') || trimmed.match(/^\d+\./)) {
        const listItems = trimmed.split(/(?=-|•|\d+\.)/).filter(item => item.trim());
        
        return (
          <ul key={index} className="space-y-3 mb-4">
            {listItems.map((item, itemIndex) => (
              <li key={itemIndex} className="flex items-start">
                <span className="text-green-600 mr-3 mt-1.5">•</span>
      ...
      // Détecter les titres (format Markdown simple)
      if (trimmed.startsWith('## ')) {
        return (
          <h3 key={index} className="font-bold text-gray-800 mb-4 mt-6 text-xl">
            {trimmed.replace('## ', '')}
          </h3>
        );
      }
      ...
      if (trimmed.startsWith('# ')) {
        return (
          <h2 key={index} className="font-bold text-gray-800 mb-4 mt-6 text-2xl">
            {trimmed.replace('# ', '')}
          </h2>
        );
      }
...
// --- Composant LeaderCard avec design amélioré ---
const LeaderCard = ({ 
  name, 
  title, 
  imageUrl, 
  ...
  <div className={`flex flex-col lg:flex-row gap-8 lg:gap-12 bg-white rounded-2xl shadow-xl overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 border border-gray-100 ${isReversed ? 'lg:flex-row-reverse' : ''}`}>
    {/* Section Photo */}
    <div className="lg:w-2/5 relative">
      <div className="relative h-80 lg:h-full min-h-[320px]">
        <Image
    ...
    <div className="lg:w-3/5 p-8 lg:p-10 flex flex-col justify-center">
      <div className="prose prose-lg max-w-none">
        {typeof children === 'string' ? (
          <StructuredBio content={children} />
        ) : (
          <div className="text-center py-8">
            <p className="text-gray-500 italic">
              Biographie en cours de rédaction...
