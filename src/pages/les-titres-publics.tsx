...
// // pages/titres-publics/les-titres-publics.tsx
...
// import { 
//   ChartBarIcon, CalendarIcon, DocumentTextIcon, PlayCircleIcon, 
//   ArrowTrendingUpIcon, BanknotesIcon, ClockIcon, CurrencyDollarIcon, EyeIcon } from '@heroicons/react/24/outline';
// import Footer from '@/components/Footer';
// import { getChiffresCles, getDerniersCommuniques, getDernieresEmissions, getDerniersResultats  } from '@/lib/api';
// import { ChiffreCle, Communique, EmissionAccueil, SupabaseChiffreCle, SupabaseCommunique, SupabaseEmission, Resultat } from '@/types/titres-publics';
...
//   const [dernieresEmissions, setDernieresEmissions] = useState<EmissionAccueil[]>([]);
//   const [derniersResultats, setDerniersResultats] = useState<Resultat[]>([]);
...
//         // Mapping des chiffres clés
//         const chiffresFormatted: ChiffreCle[] = chiffresData.map((item: SupabaseChiffreCle, index) => ({
//           id: item.id,
//           label: item.label,
//           valeur: item.valeur,
//           date: item.date_mise_a_jour,
...
//         setChiffresCles(chiffresFormatted);
//         setRapports(rapportsData as Report[]);

//         // Mapping des communiqués
//         const communiquesFormatted: Communique[] = communiquesData.map((item: SupabaseCommunique) => ({
//           id: item.id,
...
//           fichier: item.url_fichier
//         }));
//         setCommuniques(communiquesFormatted);

//         // Mapping des dernières émissions
//         const emissionsFormatted: EmissionAccueil[] = emissionsData.map((item: SupabaseEmission) => ({
//           id: item.id,
//           type: item.type as 'BTA' | 'OTA',
//           designation: item.designation,
//           dateSouscription: item.date_souscription,
...
//           statut: item.statut 
//         }));
//         setDernieresEmissions(emissionsFormatted);

//         // Mapping des résultats
//         const resultatsFormatted: Resultat[] = resultatsData.map((item) => ({
//           id: item.id,
//           emissionId: item.emission_id,
//           titre: item.emissions?.titre || 'Titre non disponible',
//           designation: item.emissions?.designation || null,
//           type: item.emissions?.type || 'BTA',
//           codeEmission: item.emissions?.isin || 'N/A',
//           dateSeance: item.emissions?.date_souscription ? 
//             new Date(item.emissions.date_souscription).toLocaleDateString('fr-FR', { 
//               day: '2-digit', 
//               month: 'long', 
//               year: 'numeric' 
//             }) : 'Date non disponible',
//           nombreSvtReseau: item.nombre_svt_reseau,
//           nombreSvtSoumissionnaires: item.nombre_svt_soumissionnaires,
//           montantAnnonce: item.emissions?.volume_emission_annonce || null,
//           montantTotalSoumissions: item.montant_total_soumissions,
//           montantTotalServi: item.montant_total_servi,
//           prixMaximumPropose: item.prix_maximum_propose,
//           prixMinimumPropose: item.prix_minimum_propose,
...
//           tauxCouverture: item.taux_couverture
//         }));
//         setDerniersResultats(resultatsFormatted);
...
//   // Filtrage des émissions par type
//   const emissionsBTA = dernieresEmissions.filter(em => em.type === 'BTA').slice(0, 3);
//   const emissionsOTA = dernieresEmissions.filter(em => em.type === 'OTA').slice(0, 2);

//   //==================================================================
//   // NOUVELLE LOGIQUE POUR LES FICHES TITRES
//   // ==================================================================
//   const fichesTitresDisponibles: { code: string; label: string }[] = useMemo(() => {
//     const typesUniques = new Set(dernieresEmissions.map(em => em.type));
    
//     const fiches = Array.from(typesUniques).map(type => {
//       let label = '';
//       switch (type) {
//         case 'BTA':
//           label = 'Bons du Trésor';
//           break;
//         case 'OTA':
//           label = 'Obligations du Trésor';
//           break;
//           default:
//           label = 'Titre Inconnu';
//       }
...
//   // ==================================================================
//   // NOUVELLE LOGIQUE POUR LE CALENDRIER PRÉVISIONNEL
//   // ==================================================================
//   const emissionsAVenir = useMemo(() => {
...
//                     derniersResultats.map((resultat: Resultat) => (
//                       <div key={resultat.id} className="border-l-4 border-yellow-400 pl-4 py-3 hover:bg-gray-50 transition-colors">
                        
//                         <div className="flex justify-between items-center">
//                           <div>
//                             <div className="text-sm text-gray-500 mb-1">{resultat.dateSeance}</div>
//                             <h3 className="font-semibold text-gray-900 mb-1">
//                               {resultat.designation || resultat.titre}
//                             </h3>
//                             <div className="text-sm text-gray-600">
...
//                             <div className="text-lg font-bold text-green-600">
//                               {resultat.montantTotalServi ? 
//                                 `${(resultat.montantTotalServi / 1000).toLocaleString('fr-FR')} Md` : 
//                                 'N/A'
//                               }
//                             </div>
//                             <div className="text-sm text-gray-500">
...
//               {/* Publications - Section UNIFORMISÉE avec seul bouton "Consulter" */}
//               <section className="bg-white rounded-xl shadow-sm p-6">
//                 <div className="flex items-center justify-between mb-6">
//                   <h2 className="text-2xl font-bold text-gray-900">Publications</h2>
//                   <Link 
...
//                         {/* SEUL BOUTON "CONSULTER" - Ouvre le document dans un nouvel onglet */}
//                         <div className="mt-4">
//                           <button 
//                             onClick={() => window.open(rapport.fileUrl, '_blank', 'noopener,noreferrer')}
//                             className="w-full bg-green-600 text-white text-center py-3 px-4 rounded-md font-medium hover:bg-green-700 transition-colors text-sm flex items-center justify-center"
...
//                           </div>
//                           <div className="text-sm text-gray-600 space-y-1">
//                             <div>Volume d&apos;émission: {adj.volumeEmissionAnnonce ? `${adj.volumeEmissionAnnonce.toLocaleString('fr-FR')} (en millions de FCFA)` : 'N/A'}</div>
//                             <div>Souscription: {formatDate(adj.dateSouscription)}</div>
//                           </div>
//                         </div>
//                       ))
...
//                 </p>
//                 <div className="space-y-2">
//                   {fichesTitresDisponibles.map((fiche) => (
//                     <Link 
//                       key={fiche.code}
//                       href={`/titres-publics/fiches/${fiche.code.toLowerCase()}`}
//                       className="flex items-center justify-between p-3 rounded-lg border border-gray-200 hover:border-green-300 hover:bg-green-50 transition-colors group"
//                     >
//                       <span className="font-medium text-gray-900 group-hover:text-green-700">
//                         {fiche.label}
