...
// // pages/admin/institutionnel/ministre.tsx
...
// import { createPagesServerClient } from '@supabase/auth-helpers-nextjs';
// import toast from 'react-hot-toast';
// // MODIFIÉ : On importe la fonction que VOUS avez créée
// import { getPersonnelByRole } from '@/lib/api-admin'; 
// import type { User } from '@supabase/supabase-js';

// // MODIFIÉ : Ce type correspond à votre table `personnel`
// interface PersonnelData {
//   role: string;
//   name: string | null;
//   title: string | null;
...
//   // MODIFIÉ : Les états correspondent aux colonnes de votre table `personnel`
//   const [name, setName] = useState(personnelData.name || '');
//   const [title, setTitle] = useState(personnelData.title || '');
//   const [bio, setBio] = useState(personnelData.bio || '');
//   const [imageUrl, setImageUrl] = useState(personnelData.imageUrl || null);
...
//   // NOUVEAU : Fonction pour gérer la sélection d'un nouveau fichier
//   const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
//     const file = event.target.files?.[0];
//     if (file) {
//       setNewImageFile(file);
...
//   // NOUVEAU : Fonction pour gérer la suppression de l'image
//   const handleRemoveImage = () => {
//     setNewImageFile(null);
//     setImageUrl(null);
//     setImagePreview(null);
...
//   // MODIFIÉ : La fonction de sauvegarde est adaptée à votre structure
//   const handleSave = async () => {
//     setLoading(true);
//     const toastId = toast.loading("Sauvegarde en cours...");
//     let finalImageUrl = imageUrl;
...
//       // MODIFIÉ : On envoie le 'role' pour identifier la ligne à mettre à jour
//       const updateResponse = await fetch('/api/institutionnel/update', {
//         method: 'PUT',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({ 
//           role: personnelData.role, // On envoie le rôle pour savoir qui modifier
//           name: name,
//           title: title,
//           bio: bio,
//           imageUrl: finalImageUrl,
...
//       <Head>
//         <title>Gestion du Ministre | Administration</title>
//       </Head>

//       {/* Dans le bloc return() de GestionMinistrePage */}
//         <header className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-10">
//             <div className="container mx-auto px-6 py-4 flex justify-between items-center">
//                 <div>
//                 <h1 className="text-2xl font-bold text-gray-800">Gestion institutionnelle</h1>
//                 <Link href="/admin/institutionnel" className="text-sm font-medium text-gray-600 hover:text-gray-800 flex items-center gap-1 mt-1">
//                     <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
//                     Retour à la liste
//                 </Link>
//                 </div>
//                 {/* C'est ici qu'on utilise la variable 'user' */}
//                 <div className="flex items-center gap-4">
...
//           <div className="mb-6">
//             <h2 className="text-xl font-semibold text-gray-900">Gestion du Ministre</h2>
//             <p className="text-gray-600">Mettre à jour les informations de la page &quot;Mot du Ministre&quot;.</p>
//           </div>
...
//             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//               <div>
//                 <label htmlFor="name" className="block text-sm font-medium text-gray-700">Nom du Ministre</label>
//                 <input type="text" id="name" value={name} onChange={(e) => setName(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm" />
//               </div>
//               <div>
//                 <label htmlFor="title" className="block text-sm font-medium text-gray-700">Titre Officiel</label>
//                 <input type="text" id="title" value={title} onChange={(e) => setTitle(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm" />
//               </div>
//             </div>

//             {/* BLOC DE CODE COMPLET POUR LA GESTION DE LA PHOTO */}
//             <div>
//               <label className="block text-sm font-medium text-gray-700">Photo du Ministre</label>
//               <div className="mt-2 flex items-center gap-6">
//                 <div className="w-32 h-32 bg-gray-100 rounded-md flex items-center justify-center relative group">
//                   {imagePreview ? (
//                     <>
//                       <Image 
//                         src={imagePreview} 
...
//                         height={400}
//                         />
//                       <button onClick={handleRemoveImage} className="absolute top-1 right-1 z-10 w-6 h-6 bg-black/60 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600" title="Supprimer l'image">
...
//                 </div>
//                 <div>
//                   <label htmlFor="photoUpload" className="cursor-pointer px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
//                     {imagePreview ? "Changer la photo" : "Uploader une photo"}
//                   </label>
//                   <input id="photoUpload" type="file" accept="image/*" onChange={handleImageChange} className="sr-only" />
//                   <p className="mt-2 text-xs text-gray-500">PNG, JPG, WEBP jusqu&apos;à 5MB.</p>
//                 </div>
//               </div>
//             </div>
...
//             {/* Champ Biographie */}
//             <div>
//               <label htmlFor="bio" className="block text-sm font-medium text-gray-700">Biographie / Mot du Ministre</label>
//               <textarea id="bio" rows={10} value={bio} onChange={(e) => setBio(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm"></textarea>
//             </div>

//             {/* Bouton de sauvegarde */}
//             <div className="pt-4 flex justify-end">
//               <button onClick={handleSave} disabled={loading} className="px-6 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 disabled:bg-gray-400">
...
//   // MODIFIÉ : On utilise VOTRE fonction pour récupérer le personnel avec le rôle 'ministre'
//   const personnelData = await getPersonnelByRole('ministre');
//   if (!personnelData) { return { notFound: true }; }
