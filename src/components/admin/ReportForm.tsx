// src/components/admin/ReportForm.tsx (AVEC UPLOAD DIRECT À SUPABASE)
...
export const ReportForm = ({ report }: ReportFormProps) => {
  // AJOUT 2 : On appelle le hook pour obtenir un client "conscient de la session"
  ...
  // ====================================================================
  // === FONCTION HANDLESUBMIT ENTIÈREMENT MISE À JOUR ===
  // ====================================================================
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (pdfFile) {
        toast.loading('Upload du fichier PDF en cours...', { id: toastId });
        
        // On crée un nom de fichier unique pour éviter les conflits
        const fileName = `${Date.now()}-${pdfFile.name.replace(/\s/g, '_')}`;

        // AJOUT 3 : On utilise le NOUVEAU client `supabase` du hook
        // Ce client enverra automatiquement le bon token d'authentification
        ...
        // // On envoie le fichier DIRECTEMENT à Supabase Storage
        // const { data, error: uploadError } = await supabase.storage
        //   .from('rapports-publics') // ASSUREZ-VOUS QUE CE NOM DE BUCKET EST CORRECT
        //   .upload(fileName, pdfFile);

        // if (uploadError) {
        //   throw uploadError; // Lève une erreur si l'upload échoue
        // }
        
        // On récupère l'URL publique du fichier qui vient d'être uploadé
      //   const { data: { publicUrl } } = supabase.storage
      //     .from('rapports-publics') // MÊME NOM DE BUCKET
      //     .getPublicUrl(data.path);
        
      //   finalFileUrl = publicUrl; // On met à jour l'URL du fichier
      // }
      
      // On vérifie qu'on a bien une URL de fichier avant de continuer
      if (!finalFileUrl) {
      ...
      toast.loading('Sauvegarde des informations du rapport...', { id: toastId });

      // ÉTAPE 2 : On envoie les données de texte et l'URL à notre API
      ...
      // On vérifie si la réponse de notre API est un succès
      const result = await response.json();
      if (!response.ok) {
        throw new Error(result.message || 'Échec de la sauvegarde du rapport');
      }
      
      toast.success(`Rapport ${report ? 'modifié' : 'créé'} avec succès !`, { id: toastId });
      
      // Si c'est une création, on vide le formulaire
      if (!report) {
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-gray-200/60 overflow-hidden">
          <form onSubmit={handleSubmit} className="p-8 space-y-8">
            ...
            <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
              ...
              >
                {loading ? 'Sauvegarde...' : (report ? 'Mettre à jour' : 'Créer le rapport')}
              </button>
            </div>
          </form>
        </div>
